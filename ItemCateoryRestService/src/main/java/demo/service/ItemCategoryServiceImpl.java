 ...
 * <li>generateItemsOfEachCategory : Generate 10 items of each category � (Item
 * 1, Item 2, ...., Item 10).</li>
...
public class ItemCategoryServiceImpl implements demo.service.intrfc.ItemCategoryService {
	...
	private static Map<String, ArrayList<String>> responseForOptimizedItems = new HashMap<String, ArrayList<String>>();
	static {
		...
		try {
			for (int catId = 1; catId <= Integer.parseInt(
				...
				for (int itmId = 1; itmId <= Integer.parseInt(
					...
					itemStore.add(item);
					itemCoordinates = String.valueOf(categoryId).concat(":").concat(item.getItemId());
	 ...
	 */
	public List<String> generateItemsOfEachCategory(String categoryId) {
	...
	public Map<String, ArrayList<String>> pickOptimizedItems() {
		...
		optimizedItemList.forEach(item -> {
			coordinateList.add(item.getItemId());
		});

		responseForOptimizedItems.put("Generated optimized items ::", (ArrayList<String>) coordinateList);
		return responseForOptimizedItems;
	}
	...
	private String printCoordinatesOfSelectedItems(List<Item> items) {
		...
		List<String> coordinateList = new ArrayList<String>();
		coordinateList.add(strItemCoordinates);
		responseForOptimizedItems.put("Coordinates of the selected items :: ", (ArrayList<String>) coordinateList);
		strItemCoordinates = null;
	...
	private List<Item> getTotalCostWithOptimizedRating(Set<Item> list) {
		...
		List<String> costAndRating = new ArrayList<String>();
		costAndRating.add("Total Cost of the selected items :: " + finalOptimizedCost);
		costAndRating.add("Total Rating of the selected items :: " + finalOptimizedRating);
		responseForOptimizedItems.put("Total Cost & Rating of the selected items ::",
				(ArrayList<String>) costAndRating);
