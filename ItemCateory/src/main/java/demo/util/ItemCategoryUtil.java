...
public class ItemCategoryUtil {
	...
	public static Set<String> getRandomItemsForEachCategory(Map<String, ArrayList<Item>> itemOfEachCategory) {
		...
		itemOfEachCategory.forEach((key, value) -> {
			Random rand = new Random();
			Item item = value.get(rand.nextInt(value.size()));
			uniqueItems.add(key.concat(ItemCategoryConstants.DEMO_ITEM_SPLITER_COLON).concat(item.getItemId()));
		});
