...
/**
 * This logger is created to support logging wherein separate log files can be
 * generated for the same logger category for separate entities.
 * 
 * @author AnilT
 */
public class EntityLogger {
	...
	/**
	 * Public interface of the logger class which returns the multi-entity
	 * logger
	 * 
	 * @param category
	 * @return
	...
	/**
	 * This method formats message with the provided parameters. It first
	 * creates a pattern which considers all the inputs passed to it.
	 * Thereafter, it uses the java.text.MessageFormatter to format the message
	 * with all parameters included in the message before returning the message
	 * to be logged.
	 * 
	 * @param message
	 *            String
	...
	public String formatMessage(String message, Object... params) {
		...
		StringBuffer messageBuffer = new StringBuffer("[< ").append(generateUniqueReferenceNumber()).append(" >] ")
				.append("\t").append(message);
		try {
			return String.format(messageBuffer.toString(), params);
		} catch (Throwable e) {
			message = messageBuffer.toString();
			instance.getLogger(THIS_COMPONENT_NAME).warning(
					"Error occurred while formatting message. Unformatted message being returned is : " + message);
			return message;
		}
	}
